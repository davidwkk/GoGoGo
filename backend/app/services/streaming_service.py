...
async def finalize_trip_plan(
    ...
    context_prompt = (
        ...
        "- You MUST have BOTH outbound AND return flights in the itinerary. Never proceed if either is missing.\n"
        "- Copy these fields DIRECTLY from search_flights results without modification:\n"
        "  departure_airport, departure_airport_name, arrival_airport, arrival_airport_name,\n"
        "  departure_time, arrival_time, duration_minutes, airplane, travel_class, price, booking_url\n"
        "- The booking_url field MUST be a valid Google Flights search URL in this exact format:\n"
        "  'https://www.google.com/travel/flights/search?q=...' (constructed from flight segments)\n"
        "- Price must be in HKD and should match what the search_flights tool returned.\n\n"
        "## Hotel Data (CRITICAL)\n"
        "- Copy these fields DIRECTLY from search_hotels results without modification:\n"
        "  hotel_class_int, reviews, location_rating, amenities, description, star_rating,\n"
        "  guest_rating, image_url, check_in_time, check_out_time, embed_map_url, booking_url\n"
        "- The booking_url field MUST be a valid booking URL.\n"
        "- The image_url field MUST be used DIRECTLY from the search_hotels tool result — do NOT generate or fetch your own image.\n\n"
        "## Attraction Data (CRITICAL)\n"
        "- Copy these fields DIRECTLY from get_attraction results without modification:\n"
        "  wiki_url, map_url (ONLY if the attraction has a specific physical location like a temple, tower, museum, theme park — DO NOT include map_url for vague areas like 'shopping street', 'night market', 'shopping district'), opening_hours, admission_fee_hkd, rating, review_count, tips, image_url, thumbnail_url, booking_url, address\n"
        "- LOCATION: Provide a specific, concrete location for each attraction — do NOT just repeat the city name.\n"
        "  Examples:\n"
        "  - 'Senso-ji Temple, Asakusa, Tokyo' instead of 'Tokyo'\n"
        "  - 'Meiji Shrine, Harajuku, Tokyo' instead of 'Tokyo'\n"
        "  - 'TeamLab Borderless, Odaiba, Tokyo' instead of 'Tokyo'\n"
        "  - 'Fushimi Inari Shrine, Kyoto' instead of 'Kyoto'\n"
        "  - 'Angkor Wat, Siem Reap, Cambodia' instead of 'Cambodia'\n"
        "- RATING: Assign a reasonable rating (1.0-5.0 scale) to ALL specific attraction sites.\n"
        "  Famous attractions like museums, temples, theme parks, and landmarks should have ratings of 4.0 or higher.\n"
        "  Use your knowledge to assign accurate ratings when the tool doesn't return one.\n"
        "- Null allowed fields (only these can be null): map_url, opening_hours, admission_fee_hkd, rating, review_count, booking_url\n"
        "- theme field is REQUIRED — never leave it null when finalizing a trip plan.\n\n"
...
def _build_system_instruction(preferences: dict | None = None) -> str:
    ...
    return (
        ...
        "- ATTRACTION LOCATION: For each attraction, provide a specific location name that is more detailed than just the city.\n"
        "  Example: instead of 'Tokyo', use 'Shibuya, Tokyo' or 'Asakusa, Tokyo' or 'Near Tokyo Tower'.\n"
        "  Use your knowledge to provide accurate, specific locations for famous attractions.\n"
