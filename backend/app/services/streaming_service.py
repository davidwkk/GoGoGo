...
async def finalize_trip_plan(
    ...
    context_prompt = (
        ...
        "- title (REQUIRED): descriptive format — include date, destination, type, traveler count.\n"
        "  Examples: '3-Day Tokyo Family Vacation Apr 1-3 | 4 People', '2-Person Bali Romantic Getaway Jun 15-20'\n"
        "- theme (REQUIRED): never leave null.\n"
        "- All enrichment fields (see Data Rules below): copy DIRECTLY from tool results without modification.\n\n"
        "## Data Rules\n"
        "### Flights (CRITICAL)\n"
        "- You MUST have BOTH outbound AND return flights in the itinerary.\n"
        "- Copy DIRECTLY from search_flights: departure_airport, departure_airport_name, arrival_airport,\n"
        "  arrival_airport_name, departure_time, arrival_time, duration_minutes, airplane, travel_class, price, booking_url.\n"
        "- booking_url must be a valid Google Flights URL: 'https://www.google.com/travel/flights/search?q=...'.\n"
        "- Price must be in HKD and match the search_flights result.\n\n"
        "### Hotels (CRITICAL)\n"
        "- Copy DIRECTLY from search_hotels: hotel_class_int, reviews, location_rating, amenities, description,\n"
        "  star_rating, guest_rating, image_url, check_in_time, check_out_time, embed_map_url, booking_url.\n"
        "- image_url: use DIRECTLY from the search_hotels tool result — do NOT generate or fetch your own image.\n"
        "- booking_url must be a valid booking URL.\n\n"
        "### Attractions (CRITICAL)\n"
        "- Copy DIRECTLY from get_attraction: wiki_url, map_url, opening_hours, admission_fee_hkd, rating,\n"
        "  review_count, tips, image_url, thumbnail_url, booking_url, address.\n"
        "- map_url: ONLY for specific physical locations (temple, tower, museum, theme park).\n"
        "  DO NOT include for vague areas like 'shopping street', 'night market', 'shopping district'.\n"
        "- LOCATION: be specific — 'Senso-ji Temple, Asakusa, Tokyo' not just 'Tokyo'.\n"
        "- RATING: assign 1.0-5.0 to ALL attractions. Famous sites (museums, temples, landmarks) ≥ 4.0.\n"
        "- Null allowed: map_url, opening_hours, admission_fee_hkd, rating, review_count, booking_url.\n\n"
        "## Tips (CRITICAL — AT LEAST 3 PER FULL DAY)\n"
        "- You MUST generate AT LEAST 1 tip for EVERY activity in days[].morning[], days[].afternoon[], and days[].evening[].\n"
        "- This means AT LEAST 3 tips per full travel day (1 per time slot).\n"
        "- Field path: days[].morning[].tips, days[].afternoon[].tips, days[].evening[].tips — each is list[str] | None.\n"
        "- Generate tips based ONLY on tool result data (reviews, ratings, descriptions, opening_hours, weather_summary).\n"
        "- Include: best time to visit, optimal duration, photography spots, dress code, weather advice, insider knowledge.\n"
        "- ALWAYS factor in weather: rain → indoor alternatives/umbrella; hot → hydration/shade; cold → warm layers.\n"
        "- NEVER leave tips null or empty — every activity MUST have at least 1 tip.\n\n"
        "## Price Hallucination Prevention (CRITICAL)\n"
        "- Do NOT invent, estimate, or make up ANY prices.\n"
        "- admission_fee_hkd: ONLY use values from tool results. Set to null if not provided.\n"
        "- Do NOT guess prices for attractions, hotels, or flights.\n\n"
        "## Image Rules\n"
        "- Hotel image_url: use DIRECTLY from search_hotels tool result.\n"
        "- Attraction image_url/thumbnail_url: use DIRECTLY from get_attraction tool result.\n"
        "- Do NOT generate or fetch your own images.\n\n"
        f"{prefs_section}"
    )
...
def _build_system_instruction(preferences: dict | None = None) -> str:
    ...
    return (
        ...
        "## Tool Rules\n"
        "CRITICAL: When user confirms trip generation, you MUST call tools in the EXACT ORDER below.\n"
        "1. Call search_flights TWICE for round-trips — once for outbound, once for return.\n"
        "2. Call search_hotels — get hotel info for the destination.\n"
        "3. Call get_weather — get weather forecast for the destination dates.\n"
        "4. Call get_attraction MULTIPLE times — get at least 2 attractions per full travel day.\n"
        "5. Call finalize_trip_plan ONLY after ALL of the above tools have returned with data.\n\n"
        ...
        "  IMPORTANT: Never call finalize_trip_plan if either outbound OR return flight search returns 0 results.\n"
        "  ALWAYS retry with different airport codes until you get both outbound AND return flights.\n"
        ...
        "    - You MUST have at least 1 outbound flight result AND 1 return flight result before proceeding. Do NOT call finalize_trip_plan if all flight searches return 0 results.\n"
        ...
        "- ATTRACTIONS: Call get_attraction MULTIPLE times — get at least 2 attractions per full travel day (a day with only 1 attraction is not acceptable — always plan at least 2 events/attractions per full day, while it can be just 1 activity for a half day: arriving the destination after 3pm).\n"
        ...
        "  4. Once confirmed, call ALL of: search_flights (TWICE for round-trips — must get both outbound AND return), search_hotels (must get at least 1 hotel), get_weather, get_attraction (at least 1 per travel day).\n"
        "  5. After ALL tools return successfully, call finalize_trip_plan() with NO arguments — it reads the conversation to extract trip details.\n"
        ...
        "- Flight: duration_minutes, airplane, travel_class, departure_airport_name, arrival_airport_name, price, booking_url\n\n"
        "## Price Hallucination Prevention (CRITICAL)\n"
        "- Do NOT invent, estimate, or make up ANY prices.\n"
